...
import { fetchHtmlViaProxy, imageProxyUrl } from './net.js?v=2.2';
...
export async function translatePageHeadless(imageUrl, settings) {
  const { cacheGet, cacheSet } = await import('./cache.js?v=2.2');
  const key = imageUrl;
...
export async function preloadChapter(chapterUrl, settings, onProgress, isCancelled) {
  const { fetchHtmlViaProxy } = await import('./net.js?v=2.2');
  const { parsePageImages } = await import('./scraper.js?v=2.2');
...
export class Reader {
  ...
  async #applyAllCached() {
    const { cacheGet } = await import('./cache.js?v=2.2');
  ...
  async retranslateChapter() {
    const { cacheDelete } = await import('./cache.js?v=2.2');
    const flagged = [];
    const rest = [];
